...
class LearnerAgentReplayBuffer(VisualReplayBuffer):
    ...
    def __init__(self, gail, buffer_size, reward_noise=True,
                 initial_data={}, done_reward=None, rew_multiplier=None):
    ...
    def get_random_batch(self, batch_size, re_eval_rw=True):
        ...
        if re_eval_rw:
            ...
            if self._done_reward is not None:       # UMaze environments
                out['rew'] = self._rew_multiplier * (self._disc.get_reward(out['pre']) + self._done_reward * np.reshape(out['don'].astype(np.float32), (batch_size, -1)))
            else:
